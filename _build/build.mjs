...
// Operate on the repo root regardless of where this script was invoked.
// The tooling lives in _build/; the assets it reads/writes are at the repo
// root (variations/, assets/). The underscore prefix on _build/ is
// intentional — Cloudflare Pages' framework auto-detection then doesn't
// see a top-level package.json and won't try to run a build at deploy time.
...
async function buildJsx() {
  console.log("\n=== JSX → JS ===");
  const dir = path.join(REPO_ROOT, "variations");
  const sources = (await fs.readdir(dir)).filter(f => f.endsWith(".jsx"));
  for (const src of sources) {
    const out = src.replace(/\.jsx$/, ".js");
    // Absolute paths with forward slashes — esbuild on Windows mishandles
    // some relative paths even with chdir set.
    const inPath = path.join(dir, src).replace(/\\/g, "/");
    const outPath = path.join(dir, out).replace(/\\/g, "/");
    await esbuildBuild({
      entryPoints: [inPath],
      outfile: outPath,
      bundle: false,
...
async function buildImages() {
  console.log("\n=== PNG → WebP ===");
  const dir = path.join(REPO_ROOT, "assets");
